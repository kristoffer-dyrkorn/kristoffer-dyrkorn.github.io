...
export default class PoiCollection {
  ...
  async loadPois() {
    const poiFile = `./fjell/poi/${this.tile.x}-${this.tile.y}.json`
    ...
    await fetch(poiFile).then((response) =>
      response.json().then((features) =>
        features.forEach((feature) => {
          ...
          const poiPosition = new THREE.Vector3(position[0] - this.tile.x, position[1] - this.tile.y, 10000)
          ...
          const intersection = raycaster.intersectObject(this.tile.tileMesh.geometry)
  ...
  showPois(camera, minDist, maxDist, minElev, maxElev, poiType) {
    ...
    this.poiData.forEach((feature) => {
      ...
      if (feature.type === poiType) {
        ...
        poi.rotateZ(
          270 * THREE.Math.DEG2RAD + Math.atan2(poiPosition.y - camera.position.y, poiPosition.x - camera.position.x)
        )
