...
export class NodeEditor extends EventDispatcher {
  ...
  _initMenu() {
    ...
    menu.add(previewButton).add(newButton).add(examplesButton).add(openButton).add(saveButton).add(menuButton)
  ...
  _initExamplesContext() {
    ...
    const addExample = (context, name, filename = null) => {
      ...
      context.add(new ButtonInput(name).setIcon("ti ti-file-symlink").onClick(onClickExample).setExtra(filename))
    }
