...
- If the `y` coordinates for the start and end points for an edge are equal, we don't need to scan the edge.

This happens since we fill the triangle by horizontal lines. That means we need to find the `x` coordinates of the edges while we traverse them vertically, one line at a time. However, a horizontal edge will have no vertical component.

At the same time, if an edge is horizontal, we will also know that none of the other edges will be horizontal - or else the triangle would have had zero area. In that case, the check that ensures the determinant is positive (ie the triangle is front-facing) would have rejected this triangle.

So, in this case, we know that the other two edges will need to be scan converted. This means we will get the needed start and end `x` coordinate values for the entire triangle, and that it can be drawn in the normal manner.
