...
If an edge is horizontal, we will also know that none of the other edges will be horizontal - or else the triangle would have had zero area. In that case and the check of whether the determinant is positive would have kicked in and we would have jumped out of the function.
...
All that is left now, is to draw the horizontal lines. We do this by iterating over the full range of `y` coordinates, from top to bottom of the triangle - and then draw the pixels from the left edge to the right edge. We get hold of the `x` coordinates by reading them out from the `startBuffer` and `endBuffer` arrays.
