...
In the illustration here, the left part shows the detailed process: The blue pixels are left and right edges of the triangle, and the red outlines show the horizontal lines that need to be drawn. On the right you can see the resulting triangle.
...
Have look at this illustration and then this will hopefully make more sense.
...
If an edge is horizontal, we will also know that none of the other edges will be horizontal - or else the triangle would have hade zero area. In that case and the check of whether the determinant is positive would have kicked in and we would have jumped out of the function.

So, in this case, we know that the other two edges will be scan converted, so we will get the needed start and end `x` coordinate values for the entire triangle so it can be drawn normally.
...
If you look through some of the scan conversion implementations on the internet you will likely find examples that check whether the endpoints are provided in increasing `y` order, and swap vertices as needed. Or, code that checks for division by zero before calculating the edge slope.
...
All that is left now, is to draw the horizontal lines. We do this by iterating over the full range of `y` coordinates, from top to botton of the triangle - and then draw the pixels from the left edge to the right edge. We get hold of the `x` coordinates by reading them out from the `startBuffer` and `endBuffer` arrays.
...
For more background information on how the RGB values in this array end up as pixels on screen, see [section 2](../triangle-rasterizer/2) in the previous article series.
