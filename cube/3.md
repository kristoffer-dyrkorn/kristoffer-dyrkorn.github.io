...
With these changes in place, the code is 564 bytes. Not bad! The previous version of the app was 630 bytes, so introducing compression resulted in a net gain of 66 bytes although that also meant adding a decoder that was 154 bytes long.
...
To sum up: We've created a small app, rewritten the code to be more compact, and then created a self-extracting version of the app relying on an compression algorithm that is part of the PNG format. The first version of the app was 2156 bytes, and we cut this down to 564 bytes, which is around 26% of the original size. At the same time, we managed to keep most of the functionality as it was.
